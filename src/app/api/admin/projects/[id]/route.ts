...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (updates.status) {
    const row = (await db.execute({ sql: 'SELECT status FROM projects WHERE id = ?', args: [id] })).rows[0];
    oldStatus = row ? String(row.status) : null;
  }
  ...
  if (updates.status && oldStatus && oldStatus !== updates.status) {
    onStatusChanged(db, Number(id), oldStatus, updates.status).catch(e => console.error('Automation error:', e));
  }
