...
// Plain, Framer-Motion-free ROI calculator. Framer Motion's animation engine does
// not run on the /start page (every motion reveal stays frozen at opacity:0), so the
// shared VoicemailCalculator would render invisible here. This is a visible equivalent.
function RoiCalculator() {
  ...
  // Conservative: assume just 1 in 3 missed calls would have become a booked job.
  const weeklyLeak = jobValue * missedPerWeek * (1 / 3);
  const annualLeak = weeklyLeak * 52;

  return (
    <section id="leak-calculator" className="relative py-24 px-4">
      ...
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-12">
          ...
          <p className="text-gray-200 max-w-xl mx-auto text-lg leading-relaxed">
            Every call that rolls to voicemail is a job that may never call back. Run the
            math on your own shop.
          </p>
        </div>
          ...
          <div className="glass-card rounded-2xl p-7 space-y-7">
            ...
            <div>
              <label className="flex items-center justify-between text-xs uppercase tracking-[0.15em] text-gray-100 mb-3 font-semibold">
                ...
                <span className="text-circuit-300 text-lg font-bold normal-case">
                  {missedPerWeek}
                </span>
              </label>
              <input
...
export default function StartLanding() {
  return (
    <>
      ...
      {/* ROI — missed-call leak calculator so they self-quantify the loss */}
      <RoiCalculator />
